        ...
        if (!isOnline()) {
            if (useLocalStorage) {
                ...
                request.onupgradeneeded = function (event) {
                    ...
                    objectStore.add(new News("PNG/new_news.jpg", $('#label-news').val().trim(), $('#text-news').val().trim()))
                 
                }
                request.onsuccess = function (event) {
                    var db = event.target.result;
                    var objectStore = db.transaction(["news"], "readwrite").objectStore("news");
                    request = objectStore.add(new News("PNG/new_news.jpg", $('#label-news').val().trim(), $('#text-news').val().trim()))
                    request.onsuccess = function (event) {
